...
export function buildPinHtml(
  ...
  return [
    ...
    `<div style="position:absolute;left:${imgL}px;top:${imgT}px;width:${imgS}px;height:${imgS}px;border-radius:50%;overflow:hidden;background:var(--color-primitive-gray-40);">`,
    `<img src="${imageUrl}" alt="" draggable="false" style="width:100%;height:100%;object-fit:cover;pointer-events:none;"/>`,
    `</div>`,
    badge,
    `</div>`,
    `</div>`,
