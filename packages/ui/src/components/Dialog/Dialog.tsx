...
function DialogRoot(props: ComponentPropsWithoutRef<typeof BaseDialog.Root>) {
...
function Content({
  ...
  return (
    <BaseDialog.Portal>
      <BaseDialog.Backdrop className="fixed inset-0 z-(--z-index-overlay) bg-semantic-system-black/60 transition-opacity duration-200 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0" />
      <BaseDialog.Popup
        initialFocus={initialFocus}
        finalFocus={finalFocus}
        className={cn(
          'fixed top-1/2 left-1/2 z-(--z-index-overlay) flex w-[calc(100%-40px)] max-w-90 translate-y-0 [transform:translate(-50%,-50%)] flex-col items-center gap-5 rounded-2xl bg-semantic-system-white p-6 text-center',
